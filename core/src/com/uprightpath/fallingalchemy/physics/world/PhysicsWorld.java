...
public class PhysicsWorld {
    ...
    private Array<TouchableEntity> touchables = new Array<TouchableEntity>();
    ...
    public void update() {
        ...
        for (int i = 0; i < agents.size; i++) {
            ...
            agentEntity.getLeftBoundingSide();
            agentEntity.getRightBoundingSide();
        ...
        for (int i = 0; i < agents.size; i++) {
            ...
            for (int j = 0; j < touchables.size; j++) {
                if (Intersector.overlapConvexPolygons(agentEntity.getCollisionPolygon(), touchables.get(j).getCollisionPolygon(), mtv)) {
                    Effect touchEffect = touchables.get(j).collisionEffect();
                    if (touchEffect != null && touchEffect.canApplyEffect(agentEntity)) {
                        touchEffect.applyEffect(agentEntity);
                    }
                }
            }
    ...
    public void cleanUp() {
        Array<AgentEntity> agentCleanUp = new Array<AgentEntity>();
        for (int i = 0; i < agents.size; i++) {
            if (agents.get(i).isDestroyedThisTurn()) {
                agentCleanUp.add(agents.get(i));
            }
        }
        agents.removeAll(agentCleanUp, true);

        Array<CollisionEntity> collisionCleanUp = new Array<CollisionEntity>();
        for (int i = 0; i < collisions.size; i++) {
            if (collisions.get(i).isDestroyedThisTurn()) {
                collisionCleanUp.add(collisions.get(i));
            }
        }
        collisions.removeAll(collisionCleanUp, true);

        Array<PlatformEntity> platformCleanUp = new Array<PlatformEntity>();
        for (int i = 0; i < platforms.size; i++) {
            if (platforms.get(i).isDestroyedThisTurn()) {
                platformCleanUp.add(platforms.get(i));
            }
        }
        platforms.removeAll(platformCleanUp, true);

        Array<TouchableEntity> touchableCleanUp = new Array<TouchableEntity>();
        for (int i = 0; i < touchables.size; i++) {
            if (touchables.get(i).isDestroyedThisTurn()) {
                touchableCleanUp.add(touchables.get(i));
            }
        }
        touchables.removeAll(touchableCleanUp, true);
    }
    ...
    public Array<TouchableEntity> getTouchables() {
        return touchables;
    }
