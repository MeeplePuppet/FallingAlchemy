package com.uprightpath.fallingalchemy.physics.builders;

import com.badlogic.gdx.math.Polygon;
import com.badlogic.gdx.math.Vector2;
import com.badlogic.gdx.math.Vector3;
import com.uprightpath.fallingalchemy.controls.Control;
import com.uprightpath.fallingalchemy.physics.world.*;
...
public class WorldBuilder {
    ...
    public void buildSurface(String name, PhysicsWorld physicsWorld, Vector3[] surfaceVectors, boolean isCollides) {
        buildSurface(name, physicsWorld, surfaceVectors, isCollides, .01f);
    }

    public void buildSurface(String name, PhysicsWorld physicsWorld, Vector3[] surfaceVectors, boolean isCollides, float slipModifier) {
        ...
        for (int i = 0; i < surfaceVectors.length - 1; i++) {
            ...
            platformEntity.getSlip().scl(slipModifier);
    ...
    public void buildUnevenSurface(String name, PhysicsWorld physicsWorld, Vector3[] surfaceVectors, boolean isCollides) {
        buildUnevenSurface(name, physicsWorld, surfaceVectors, isCollides, .01f);
    }

    public void buildUnevenSurface(String name, PhysicsWorld physicsWorld, Vector3[] surfaceVectors, boolean isCollides, float slipModifier) {
        RailSeries surface = new RailSeries(name);
        PlatformEntity platformEntity;
        Vector3 first, second;
    ...
    public void buildPlatform(String name, PhysicsWorld physicsWorld, Vector3[] surfaceVectors) {
        buildPlatform(name, physicsWorld, surfaceVectors, .01f);
    }

    public void buildPlatform(String name, PhysicsWorld physicsWorld, Vector3[] surfaceVectors, float slipModifier) {
        RailSeries surface = new RailSeries(name);
        PlatformEntity platformEntity;
        Vector3 first, second;
        for (int i = 0; i < surfaceVectors.length - 1; i++) {
    ...
    public void buildStairs(String name, PhysicsWorld physicsWorld, Vector3[] surfaceVectors) {
        ...
        edgePlatformEntity.setEndPoints(new Vector2(surfaceVectors[surfaceVectors.length - 1].x, surfaceVectors[surfaceVectors.length - 1].y), new Vector2(surfaceVectors[surfaceVectors.length - 1].x + 1.f, surfaceVectors[surfaceVectors.length - 1].y));
    ...
    public void buildLadder(String name, PhysicsWorld physicsWorld, Vector2[] ladderVectors) {
        ...
        for (int i = 0; i < ladderVectors.length - 1; i++) {
            ...
            ladderEntity.setCollisionPolygon(new Polygon(new float[]{second.x - .01f, second.y, first.x - .01f, first.y, first.x + .01f, first.y, second.x + .01f, second.y}));
    ...
    public void buildWall(String name, PhysicsWorld physicsWorld, float[] wallVectors) {
        CollisionEntity collisionEntity = new CollisionEntity();
        collisionEntity.setName(name);
        collisionEntity.setCollisionPolygon(new Polygon(wallVectors));
        physicsWorld.addCollisionEntity(collisionEntity);
    }
}
