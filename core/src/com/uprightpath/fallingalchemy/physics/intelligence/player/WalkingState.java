 ...
 */
public class WalkingState extends AgentState {
    ...
    public void applyLogic(PhysicsWorld physicsWorld) {
        ...
        if (Control.JUMP.isJustDown() && Control.DOWN.isDown()) {
            ...
            playerAgent.getAcceleration().y = this.playerAgent.getAgentStats().getMovementProfile().getGroundAcceleration().y;
            playerAgent.setRail(null);
        }

        if (Control.LEFT.isDown()) {
            playerAgent.getAcceleration().x = -this.playerAgent.getAgentStats().getMovementProfile().getGroundAcceleration().x;
    ...
    public void applyLimits(PhysicsWorld physicsWorld) {
        ...
        float maxX = Math.max(Math.abs(previousVelocity.x * playerAgent.getRail().getFriction()), Math.min(physicsWorld.getGroundSpeedMax().x, this.playerAgent.getAgentStats().getMovementProfile().getMaxGroundVelocity().x));
        float maxY = Math.max(Math.abs(previousVelocity.y * playerAgent.getRail().getFriction()), Math.min(physicsWorld.getGroundSpeedMax().y, this.playerAgent.getAgentStats().getMovementProfile().getMaxGroundVelocity().y));
